...
unless (-e $csv_file) {
    die "[debug] csv no file: $csv_file\n";
}
...
# モデル情報収集
my $model_name = $row->[27] // '';
if ($model_name) {
    my $model_file = "p-${prefix}cover.xhtml";
    push @{ $combined_data{$combined_name}{models} }, [$model_name, $model_file];
}
    }
    ...
    if (exists $combined_data{$combined_name}{models}) {
        _insert_model_navigation($nav_file, $combined_data{$combined_name}{models});
    }
}
        ...
        for my $fname (qw(p-cover.xhtml p-colophon.xhtml)) {
...
sub _insert_model_navigation {
    my ($file_path, $model_entries_ref) = @_;
    open my $in, '<:utf8', $file_path or die "Can't open nav file: $file_path\n";
    my @lines = <$in>;
    close $in;
    ...
    for my $line (@lines) {
        if ($line =~ /<!--\s*▼モデル目次印字位置▼\s*-->/) {
            $inserted = 1;
            for my $entry (@$model_entries_ref) {
                my ($name, $file) = @$entry;
                push @new_lines, qq|<li><a href="xhtml/$file">$name</a></li>\n|;
            }
            # マーカー自体は出力しない
        } else {
            push @new_lines, $line;
    ...
    if ($inserted) {
        open my $out, '>:utf8', $file_path or die "Can't write updated nav file: $file_path\n";
        print $out @new_lines;
        close $out;
        print "[debug] navigation updated with model entries (marker removed): $file_path\n";
    } else {
        warn "[alert] モデル目次マーカーが見つかりません: $file_path\n";
    }
}
