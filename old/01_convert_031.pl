...
sub update_opf_template {
    ...
    for my $i (0 .. 5) {
        ...
        my $kata = $row->[14 + $i * 3] // '';
        next unless $name;
        my $id = sprintf("creator%02d", scalar @creator_blocks);
        push @creator_blocks, qq|<dc:creator id=\"$id\">$name</dc:creator>\n<meta refines=\"#$id\" property=\"file-as\">$kata</meta>|;
    }
    for (@lines) {
