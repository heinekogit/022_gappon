...
sub extract_opf_tags {
    ...
    while (my $line = <$in>) {
        # image タグ: media-type の位置を統一、id/href に prefix を付加
        ...
        # xhtml タグ: media-type の位置統一、SVG指定を追加、id/href に prefix
        ...
        # spine タグ: linear="yes" 追加、右ページ固定指定を含める（偶数か奇数で調整も可能）
        elsif ($line =~ m{<itemref[^>]+idref="p-(\d{4})"}i) {
            my $num = $1;
            my $idref = "p-$prefix$num";
            my $spread = ($num % 2 == 1) ? 'page-spread-right' : 'page-spread-left';  # 偶数奇数で左右決める
