    ...
    if ($type eq '素材') {
        ...
        if (@$spine_ref && $spine_ref->[-1] =~ /page-spread-right/) {
            ...
            if (-e $white_xhtml_template) {
                ...
                for (@white_lines) {
                    s/●タイトル名●/$title/g;
                    s/image-white\.jpg/image-${prefix}9999.jpg/g;
                }
        ...
        my @extra_templates = qw(p-id_0000.xhtml p-white_0000.xhtml);
        for my $file (@extra_templates) {
            my $src = File::Spec->catfile($template_dir, $file);
            my $dst = File::Spec->catfile($combined_dir, 'item', 'xhtml', $file);
            if (-e $src) {
                copy($src, $dst) or warn "[warn] extra template copy failed: $src -> $dst\n";
            } else {
                warn "[warn] extra template not found: $src\n";
            }
        }
