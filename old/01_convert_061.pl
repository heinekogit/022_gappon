...
while (my $row = $csv->getline($fh)) {
    ...
    if ($type eq '素材') {
        ...
        my $cover_xhtml_name  = "p-${prefix}id.xhtml";
        ...
        if (-e $cover_image_src && -e $id_cover_template) {
            ...
            for (@id_lines) {
                s/●タイトル名●/$title/g;
                s/image-id_cover\.jpg/$cover_image_name/g;
            }
            ...
            unshift @$xhtml_ref, qq|<item id="p-${prefix}id" href="xhtml/$cover_xhtml_name" media-type="application/xhtml+xml" properties="svg"/>\n|;
            unshift @$spine_ref, qq|<itemref linear="yes" idref="p-${prefix}id" properties="page-spread-right"/>\n|;
        } else {
